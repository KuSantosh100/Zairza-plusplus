  ...
  const connectWallet = async () => {
    try {
      ...
      if (chainId != goerliChainId) {
        alert('You are not to connected to the goerli test net!')
  ...
  const getAllTasks = async () => {
    try {
      ...
      if (ethereum) {
        ...
        console.log('ethereum object doenot exist')
      }
    } catch (error) {
      console.log(error)
    }
  ...
  const deleteTask = key => async () => {
    try{
      ...
      if (ethereum) {
        ...
        const deleteTaskMsg = await TaskContract.deleteTask(key, true)
        console.log('deleted task', deleteTaskMsg)
        let allTasks = await TaskContract.getMyTasks()
        setTasks(allTasks)
      } else {
